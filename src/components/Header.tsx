...
const Header = () => {
  ...
  const Logo = ({ className = '', isScrolled }: { className?: string; isScrolled: boolean }) => (
    <div className={`relative ${className}`}>
      <div className="relative w-12 h-12 lg:w-14 lg:h-14">
        ...
        <div className={`absolute -top-1 -right-1 w-3 h-3 rounded-full transition-all duration-300 ${isScrolled ? 'bg-red-500' : 'bg-red-400'
          } opacity-80`}></div>
        <div className={`absolute -bottom-1 -left-1 w-2 h-2 rounded-full transition-all duration-300 ${isScrolled ? 'bg-red-400' : 'bg-red-300'
          } opacity-60`}></div>
      </div>
    </div>
  );
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-out animate-in slide-in-from-top-4 ${isScrolled
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 lg:h-20">
          ...
          <nav className="hidden lg:flex items-center space-x-1 font-sans text-base lg:text-lg">
            {navItems.map((item, index) => (
              <div key={item.name} className="relative" style={{ animationDelay: `${index * 100}ms` }}>
                <a
                  href={item.path}
                  className={`relative px-4 py-2 font-semibold transition-all duration-300 rounded-lg group ${location.pathname === item.path
        ...
        <div className={`lg:hidden overflow-hidden transition-all duration-500 ease-in-out ${isMobileMenuOpen
          ...
          <div className="py-6 bg-white/98 backdrop-blur-xl rounded-b-2xl shadow-2xl border-t border-gray-100">
            <div className="space-y-2">
              {navItems.map((item, index) => (
                ...
                >
                  <a
                    ...
                    className={`block px-6 py-3 mx-4 rounded-xl text-lg font-semibold transition-all duration-300 ${location.pathname === item.path
              ...
              <div className={`px-6 pt-4 transform transition-all duration-500 ${isMobileMenuOpen
